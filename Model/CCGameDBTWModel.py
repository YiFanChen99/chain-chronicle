...
class MyDBUpdater(object):
    ...
    def update_new_characters(self):
        self.updated_count = 0
        # 新增的角色其 ID 會 <1000
        character_names = DBAccessor.execute('select FullName from Character where ID<1000').fetchall()
        for character_name in character_names:
            try:
                # 逐一根據其 FullName 開啟更新視窗
                character = Character.create_by_cgdt_character(self.data_owner.find_character_by_full_name(character_name[0]))
            ...
            # 若資料取得發生問題，則記錄後就略過
                ...
                print e
        print '共 {0} 筆資料，其中 {1} 筆更新完成'.format(len(character_names), self.updated_count)
    ...
    def _update_new_character_with_its_id(self, character):
        ...
        CharacterModel.update_character_into_db(character), DBAccessor.commit()
